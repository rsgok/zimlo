...
- iOS 在首次完成配对且推送服务可用后请求通知权限，提醒审批、重要结果与失败。角标是真实未读数；同任务短时间内的结果/失败会合并，未处理审批只在临近过期时再提醒一次。App 在前台查看别的任务时显示轻提示，正在查看同一任务时保持安静。默认锁屏不显示任务标题，通知只携带设备端可解密的任务路由。低风险审批（无确认短语的批准一次/拒绝）支持锁屏快捷操作：APNs category 只是明文通用标识，决策 id 只放在加密路由内；高风险与需输入的审批仍只能进 App 处理。macOS 使用本机系统通知执行同一套高信号策略。两端都提供「仅关键通知」与固定 22:00–08:00 安静时段；审批和失败始终视为关键。通知权限被拒时设置页会给出去系统设置的引导，iOS 还会显示 APNs 注册环境与最近投递状态。
...
开发阶段需要用 Xcode 将原生 App 安装到模拟器或已登记真机，详细步骤见 [iOS README](apps/ios/README.md)。普通用户的目标路径是从 TestFlight / App Store 安装；不会要求运行 Mac 命令。iPhone 的后台通知使用 APNs；Mac App 的提醒由本机通知中心直接投递，不依赖 APNs。
