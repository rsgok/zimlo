...
for architecture in arm64 x86_64; do
  app_path="${release_root}/apps/${architecture}/Zimlo.app"
  dmg_path="${release_root}/Zimlo-${version}-${architecture}.dmg"
  staging_path="${release_root}/dmg-${architecture}"
  runtime_archive="${runtime_artifact_root}/ZimloRuntime-${runtime_version}-${architecture}.zip"

  ZIMLO_SKIP_PROJECT_BUILD=1 \
  ZIMLO_RELEASE=1 \
  ZIMLO_VERSION="${version}" \
  ...
  ZIMLO_APPCAST_URL="${runtime_base_url}/appcast-${architecture}.xml" \
...
echo "${release_root}/Zimlo-${version}-arm64.dmg"
echo "${release_root}/Zimlo-${version}-x86_64.dmg"
