...
function snapshot(host: Host, suffix: string): Snapshot {
  return {
    ...
    posts: [{ id: `post-${suffix}`, hostId: host.id, projectId: null, taskId: suffix, runId: suffix, agentId: suffix, sessionId: `session-${suffix}`, kind: "result", presentation: { system: "editorial", theme: "ink_classic", layout: "feature", typography: "serif", density: "balanced", mediaPlacement: "none" }, headline: suffix, takeaway: suffix, highlights: [], blocks: [], content: { type: "text" }, dedupeKey: suffix, source: "agent", createdAt: host.lastSeenAt }],
