                ...
                VStack(spacing: 0) {
                    HStack {
                        ...
                        Button(showsCloseButton ? "关闭" : "稍后", action: onDismiss)
                            ...
                            .accessibilityHint(showsCloseButton ? "返回 Zimlo" : "稍后可从 Feed 重新连接 Mac")
                    }
                    ...
                    VStack(spacing: 10) {
                        Text(returningUser ? "重新连接 Mac" : "先连接你的 Mac")
                            .font(ZFont.title)
                            .multilineTextAlignment(.center)
                            .fixedSize(horizontal: false, vertical: true)
                        Text("在 Mac 安装 Zimlo，一键接入 Codex 或 Claude Code，然后用手机扫码。大约需要 2 分钟。")
                    ...
                    VStack(spacing: 10) {
                        ...
                        Button("我已经在 Mac 打开 Zimlo", action: onReadyToPair)
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    Image(systemName: returningUser ? "desktopcomputer.trianglebadge.exclamationmark" : "desktopcomputer")
                        ...
                        .clipShape(Circle())
                        .accessibilityHidden(true)
                    Text(returningUser ? "没有已连接的 Mac" : "还没有连接 Mac")
                        .font(ZFont.title2)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                        .padding(.top, 22)
                    Text(returningUser
                        ...
                        .fixedSize(horizontal: false, vertical: true)
                        .padding(.top, 10)
                    Button(returningUser ? "重新连接 Mac" : "连接 Mac", action: onConnect)
                        .buttonStyle(PhoneSetupPrimaryButtonStyle())
                        .padding(.top, 26)
                        .frame(maxWidth: 360)
                    Text("Mac 暂时离线时不会进入这个页面，最近内容仍会保留。")
                        .font(ZFont.footnote)
                        .foregroundStyle(ZColor.muted)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
