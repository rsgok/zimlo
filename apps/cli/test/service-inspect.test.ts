...
const healthyFetch = (async () => ({
  ok: true,
  json: async () => ({ ok: true, version: "0.2.0", protocolVersion: 2 }),
...
describe("inspectService", () => {
  ...
  it("verifies ownership when descriptor and lock owner agree and healthz responds", async () => {
    ...
    expect(inspection.health?.protocolVersion).toBe(2);
...
describe("formatServiceInspection", () => {
  ...
  it("renders a running service", () => {
    const text = formatServiceInspection({
      ...
      ownership: "verified",
      portReachable: true,
      health: { ok: true, version: "0.2.0", protocolVersion: 2 },
      socketExists: true,
...
describe("isServiceOperational", () => {
  const healthy: ServiceInspection = {
    ...
    portOwner: null, health: { ok: true, version: "0.2.0", protocolVersion: 2 },
  ...
  it("requires verified ownership, a healthy endpoint, and the expected protocol", () => {
    expect(isServiceOperational(healthy, 2)).toBe(true);
    expect(isServiceOperational({ ...healthy, ownership: "unverifiable" }, 2)).toBe(false);
    expect(isServiceOperational({ ...healthy, health: { ok: false } }, 2)).toBe(false);
    expect(isServiceOperational(healthy, 3)).toBe(false);
  });
});
